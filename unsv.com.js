...
(function(window, document){
	var unsv_db = window.openDatabase('unsv', '1.0', 'unsv database', 50*1024*1024, function(db){},function(e){});
	var url = window.location.href;
	var url_md5 = md5(url);
	unsv_db.transaction(function(tx){
		/*	
		tx.executeSql("DROP TABLE IF EXISTS words", [], function(tx, res){}, function(tx, res){
        });
	...
	function get_selection(e) {
		...
		if (str !== '') {
			...
			obj.html(text);
			unsv_db.transaction(function(tx){
				tx.executeSql('SELECT * FROM words WHERE word = ? AND url_md5 = ?', [str, url_md5], function(tx, res) {
					var len = res.rows.length;
					if (len === 0) {
						tx.executeSql('INSERT INTO words (url_md5, word) VALUES (?, ?)', [url_md5, str], function(res){}, function(res){});
					}
					tx.executeSql('INSERT INTO urls (url_md5, url) VALUES (?, ?)', [url_md5, url], function(res){}, function(res){});

				}, function(tx, res){});
			});
		}
	}
	$(document).bind('mousedown', get_selection);
	//read database
	function init_word() {
		var obj = $('#articleFulltext');
		var text = obj.html();
